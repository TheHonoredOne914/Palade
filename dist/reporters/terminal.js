...
function renderCategoryScore(category, score, findingCount) {
...
export async function reportTerminal(ctx) {
    const lines = [];
    lines.push('');
    ...
    lines.push(`  Critical: ${chalk.red(debtCounts.critical.toString())} | High: ${chalk.yellow(debtCounts.high.toString())} | Medium: ${chalk.blue(debtCounts.medium.toString())} | Low: ${chalk.gray(debtCounts.low.toString())} | Info: ${chalk.gray(debtCounts.info.toString())}`);
    ...
    lines.push(chalk.dim(`Duration: ${(ctx.swarm.durationMs / 1000).toFixed(1)}s | Chunks: ${ctx.swarm.totalChunks} | Input Tokens (est): ${ctx.swarm.totalTokensEstimated.toLocaleString('en-US')}`));
    lines.push('');
...
export function printDiffBanner(ctx) {
    ...
    lines.push(`  ${chalk.bold('Comparing:')} ${chalk.cyan(ctx.headBranch)} → ${chalk.gray(ctx.baseBranch)}`);
    lines.push(`  ${chalk.bold('Changed:')}   ${ctx.changedCount} files  ${chalk.green(`(+${ctx.additions})`)} ${chalk.red(`(-${ctx.deletions})`)}`);
    lines.push('');
    return lines.join('\n');
}
export function printDiffSummary(ctx) {
    ...
    const deltaStr = score.delta === 0
        ? chalk.gray('  +0')
        : score.delta > 0
            ? chalk.green(`  +${score.delta}`)
            : chalk.red(`  ${score.delta}`);
    lines.push(`  ${chalk.bold('Diff Score:')}  ${scoreColor(score.score.toString())} ${chalk.gray('/ 100')} ${chalk.bold(`(delta: ${deltaStr} vs ${baseBranch})`)}`);
    lines.push('');
    lines.push(`  Findings in changed files: ${chalk.bold(String(findingDiff.introduced.length))} total`);
