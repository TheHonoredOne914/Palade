...
function renderCategoryScoreHtml(category, score) {
...
function renderCrossAgentFindingHtml(finding) {
    const severityClass = SEVERITY_CLASSES[finding.severity] ?? 'severity-low';
...
function renderAgentTimingHtml(timing, maxDuration) {
...
function renderFindingsSummaryHtml(findings) {
...
function renderFindingsDetailHtml(findings) {
...
function buildTemplateData(ctx) {
    ...
    const crossAgentFindingsHtml = ctx.crossAgentFindings
        ...
        .join('\n');
    const agentTimingEntries = Object.entries(ctx.swarm.agentTimings).filter((entry) => entry[1] !== undefined);
...
function replacePlaceholders(template, data, ctx) {
...
export function writeHtmlReport(ctx, outputPath) {
...
export function startLocalServer(htmlPath, port = 4242, options = {}) {
    if (htmlServer) {
        stopLocalServer();
    }
