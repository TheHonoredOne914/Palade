 ...
 */
export function isProviderConfigured(providers, id) {
...
export function setNestedValue(content, dotPath, value) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (keyMatch) {
            ...
            const matchesPath = pathStack.length === targetPath.length && targetPath.every((p, idx) => pathStack[idx] === p);
            if (matchesPath) {
    ...
    for (let prefixLen = targetPath.length; prefixLen >= 0; prefixLen--) {
        ...
        const block = [];
        missing.forEach((section, depth) => block.push(`${' '.repeat(insertIndent + 2 * depth)}${quoteKeyIfNeeded(section)}: {`));
        block.push(`${' '.repeat(insertIndent + 2 * missing.length)}${quoteKeyIfNeeded(keyName)}: ${valueStr}`);
 ...
 */
export async function saveConfigValues(projectRoot, updates) {
...
export async function saveConfigValue(projectRoot, dotPath, value) {
    return saveConfigValues(projectRoot, { [dotPath]: value });
}
export async function readCurrentKeys(projectRoot) {
    const result = {};
 ...
 */
export async function saveApiKey(projectRoot, providerId, apiKey) {
