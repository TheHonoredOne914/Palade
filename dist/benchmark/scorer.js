function fileMatches(claimFile, defectFile) {
    if (claimFile === defectFile)
        return true;
    const a = claimFile.replace(/\\/g, '/');
    const b = defectFile.replace(/\\/g, '/');
    return a.endsWith(b) || b.endsWith(a);
}
function matchDefect(claim, defects, tolerance) {
    const candidates = defects.filter((d) => fileMatches(claim.file, d.file) && Math.abs(claim.lineStart - d.lineStart) <= tolerance);
    ...
    return candidates.reduce((best, d) => {
        ...
        if (dDist < bestDist)
            return d;
        if (dDist === bestDist && d.category === 'real-bug' && best.category !== 'real-bug')
            return d;
        return best;
    });
}
export function scoreAgent(agentName, claims, defects, opts = {}) {
    const tolerance = opts.lineTolerance ?? 10;
    const matches = [];
    const foundRealBugIds = new Set();
...
export function scoreAgents(runs, defects, opts = {}) {
    ...
    for (const r of perAgent) {
        ...
        for (const m of r.matches) {
            if (m.outcome === 'tp' && m.defect)
                found.add(m.defect.id);
            if (m.outcome === 'tp')
                tp++;
        }
    }
