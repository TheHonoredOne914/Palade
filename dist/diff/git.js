        ...
        if (status !== 'deleted') {
            try {
                ...
                for (const dl of diffLines) {
                    ...
                    if (!seenHunk && (dl.startsWith('+++') || dl.startsWith('---')))
                        continue;
                    if (dl.startsWith('+'))
                        additions++;
...
export async function getMergeBase(baseBranch, cwd) {
    try {
 ...
 */
export function getFileContentAtRef(ref, filePath, cwd) {
    try {
...
export async function getBaseScore(baseBranch, historyFile, cwd) {
