...
function buildLooseFingerprint(f) {
...
function findingOverlapsAdded(finding, addedRanges) {
 ...
 */
export function scopeToDiff(findings, changedFiles) {
...
export function compareFindings(headFindings, 
...
baseFindings, changedFiles) {
    ...
    for (const f of baseInScope) {
        ...
        if (!baseByFingerprint.has(fp))
            baseByFingerprint.set(fp, []);
    ...
    const baseByLoose = new Map();
    for (const f of baseInScope) {
        if (matchedBase.has(f))
            continue;
        const loose = buildLooseFingerprint(f);
        if (!baseByLoose.has(loose))
            baseByLoose.set(loose, []);
        baseByLoose.get(loose).push(f);
    }
    const candidatePairs = [];
    for (const f of headInScope) {
        if (matchedHead.has(f))
            continue;
        ...
        for (const bf of baseMatches) {
            ...
            if (dist <= LINE_TOLERANCE)
                candidatePairs.push({ head: f, base: bf, dist });
        }
    }
    candidatePairs.sort((a, b) => a.dist - b.dist);
    for (const { head, base } of candidatePairs) {
        if (matchedHead.has(head) || matchedBase.has(base))
            continue;
...
export function rankIntroducedFindings(introduced) {
