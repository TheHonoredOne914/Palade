import { SEVERITY_PENALTY, DEFAULT_PENALTY_CAPS, DEFAULT_COMPLEXITY_PENALTIES, DEFAULT_CROSS_AGENT_PENALTY_WEIGHTS, } from '../config/defaults.js';
 ...
 */
export function applyComplexityMultiplier(complexity, penalty, thresholds = DEFAULT_COMPLEXITY_PENALTIES) {
    if (complexity < thresholds.lowThreshold)
        return penalty * thresholds.lowFactor;
    if (complexity > thresholds.highThreshold)
        return penalty * thresholds.highFactor;
    return penalty;
}
/**
 ...
 */
export function penaltyFor(f, severityWeights = SEVERITY_PENALTY) {
    ...
    return Number.isFinite(f.scorePenalty) ? f.scorePenalty : severityWeights[f.severity];
}
export function countBySeverity(findings, agentName) {
    let total = 0;
    let critical = 0;
    let high = 0;
...
export function calculateCategoryScore(findings, category, severityWeights = SEVERITY_PENALTY, complexityPenalties = DEFAULT_COMPLEXITY_PENALTIES, categoryPenaltyCap = DEFAULT_CATEGORY_PENALTY_CAP) {
...
export function computeFindingPenalty(f, severityWeights, complexityPenalties) {
...
export function calculateTotalPenalty(findings, severityWeights = SEVERITY_PENALTY, complexityPenalties = DEFAULT_COMPLEXITY_PENALTIES) {
    let penalty = 0;
    for (const f of findings) {
        penalty += computeFindingPenalty(f, severityWeights, complexityPenalties);
    }
...
export function calculateCrossAgentPenalty(crossFindings, weights = DEFAULT_CROSS_AGENT_PENALTY_WEIGHTS) {
    let penalty = 0;
    for (const cf of crossFindings) {
        let base = 0;
        if (cf.severity === 'critical')
            base = weights.critical;
        else if (cf.severity === 'high')
            base = weights.high;
        else if (cf.severity === 'medium')
            base = weights.medium;
...
export function calculateScore(findings, crossAgentFindings, previousScore = null, scoreConfig, 
/**
...
executedCategories) {
    ...
    complexityPenalties.lowFactor = Math.max(0, Number.isFinite(complexityPenalties.lowFactor)
        ? complexityPenalties.lowFactor
        : DEFAULT_COMPLEXITY_PENALTIES.lowFactor);
    complexityPenalties.highFactor = Math.max(0, Number.isFinite(complexityPenalties.highFactor)
    ...
    penaltyCaps.categoryPenaltyCap = Math.max(0, Number.isFinite(penaltyCaps.categoryPenaltyCap)
        ? penaltyCaps.categoryPenaltyCap
        : DEFAULT_PENALTY_CAPS.categoryPenaltyCap);
    penaltyCaps.totalPenaltyCap = Math.max(0, Number.isFinite(penaltyCaps.totalPenaltyCap)
    ...
    const baseCategories = executedCategories === undefined
        ? allBaseCategories
    ...
    const nonBaseExecutedCategories = (executedCategories ?? []).filter((c) => !allBaseCategories.includes(c));
    const uniqueAgents = Array.from(new Set(findings.map((f) => f.agentName)));
    const categories = Array.from(new Set([...baseCategories, ...nonBaseExecutedCategories, ...uniqueAgents]));
    const categoryScores = categories.map((cat) => calculateCategoryScore(findings, cat, severityWeights, complexityPenalties, penaltyCaps.categoryPenaltyCap));
    ...
    const penaltyScore = Math.max(TOTAL_SCORE_FLOOR, Math.round(100 - Math.min(totalPenalty, penaltyCaps.totalPenaltyCap)));
