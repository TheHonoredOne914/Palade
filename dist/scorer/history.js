import { readFileSync, writeFileSync, renameSync, mkdirSync, existsSync, copyFileSync, unlinkSync, statSync, } from 'node:fs';
...
function isValidCategoryScore(item) {
    if (typeof item !== 'object' || item === null)
        return false;
    const c = item;
    return (typeof c.category === 'string' &&
 ...
 */
export function parseHistoryEntries(raw) {
    ...
    for (const item of parsed) {
        if (typeof item === 'object' &&
            ...
            const obj = item;
            const rawBreakdown = obj.breakdown;
            const isValidBreakdown = rawBreakdown !== null &&
            ...
            const kind = obj.kind === 'diff' ? 'diff' : obj.kind === 'full' ? 'full' : undefined;
...
export function readHistory(historyPath) {
    ...
    try {
        ...
        console.error(`[palade] history.json could not be read (${err instanceof Error ? err.message : String(err)}). ` +
    ...
    try {
        ...
        console.error(`[palade] history.json is corrupt and could not be parsed (${err instanceof Error ? err.message : String(err)}). ` +
            `Backed up to ${backupPath}. Starting from empty history.`);
        return [];
    }
}
export function writeHistory(historyPath, entries, maxEntries = MAX_HISTORY_ENTRIES) {
    try {
...
async function acquireLock(historyPath) {
    const dir = dirname(historyPath);
    try {
        mkdirSync(dir, { recursive: true });
    }
...
export async function appendEntry(historyPath, entry, maxEntries = MAX_HISTORY_ENTRIES) {
    ...
    if (!lockHandle) {
        ...
        console.warn(`[palade] Could not acquire history.json lock — this run's score was not persisted to disk.`);
...
export function getPreviousScore(historyPath) {
