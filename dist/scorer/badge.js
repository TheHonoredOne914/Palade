...
export function getScoreColor(score) {
    ...
    if (clamped >= SCORE_THRESHOLDS.excellent)
        return 'brightgreen';
    if (clamped >= SCORE_THRESHOLDS.good)
        return 'green';
    if (clamped >= SCORE_THRESHOLDS.warning)
        return 'yellow';
    if (clamped >= SCORE_THRESHOLDS.poor)
        return 'orange';
    return 'red';
}
export function getBadgeData(score) {
    return {
        score: `${score}/100`,
