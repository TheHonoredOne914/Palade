...
function getValence(text) {
    ...
    if (hardenHits > relaxHits)
        return { valence: 'harden', margin };
    if (relaxHits > hardenHits)
        return { valence: 'relax', margin };
...
export function detectConflicts(findings) {
    ...
    for (const f of findings) {
        if (!f.filePath || f.lineStart === undefined || f.lineEnd === undefined)
            continue;
    ...
    for (const [filePath, fileFindings] of grouped.entries()) {
        for (let i = 0; i < fileFindings.length; i++) {
            ...
            for (let j = i + 1; j < fileFindings.length; j++) {
                ...
                const opposite = (valA.valence === 'harden' && valB.valence === 'relax') ||
...
const VerdictSchema = z.object({
    is_conflict: z
        .boolean()
        .describe('True ONLY if the two recommendations are mutually exclusive and cannot both be applied.'),
    decision: z
        .string()
...
export async function arbitrateConflict(conflict, context, signal) {
...
export async function saveDecision(projectRoot, conflict, verdict, retentionLimit = 100) {
    const slugBase = conflict.filePath
        .split('/')
        .pop()
        ?.replace(/\.[^/.]+$/, '') || 'decision';
    const hash = crypto
...
`;
    ...
    if (mdFiles.length >= retentionLimit) {
        ...
        const withMtimes = await Promise.all(mdFiles.map(async (f) => {
...
export async function checkDecisionDrift(projectRoot, changedFiles) {
    const dir = join(projectRoot, '.palade', 'decisions');
    if (!existsSync(dir))
        return [];
