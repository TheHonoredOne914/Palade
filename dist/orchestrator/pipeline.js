...
// the prompt.
export function contextBlockKey(header) {
...
export function mergeContexts(retrieved, keyword) {
    if (!retrieved)
        return keyword;
    if (!keyword)
        return retrieved;
...
export async function runPipeline(opts) {
    ...
    if (scope.symbolChunks && scope.symbolChunks.length > 0) {
        ...
        for (const chunk of chunks) {
            ...
            if (existing)
                ...
                chunksByPath.set(chunk.filePath, [chunk]);
        }
        manifests = await Promise.all(Array.from(chunksByPath.entries()).map(async ([filePath, fileChunks]) => {
        ...
        console.log(`[pipeline] Symbol-scoped: ${chunksByPath.size} file(s) → ${chunks.length} chunk(s) (~${estimateTotalTokens(chunks).toLocaleString('en-US')} tokens)`);
    }
    else {
        ...
        console.log(`[pipeline] Chunking complete: ${manifests.length} files → ${chunks.length} chunks (~${estimateTotalTokens(chunks).toLocaleString('en-US')} tokens)`);
    }
    ...
    if (scope.annotationsOnly) {
        activeChunks = activeChunks.filter((c) => annotationSummary.reviewRequests.some((r) => c.filePath === r.filePath && c.startLine <= r.line && c.endLine >= r.line) ||
            annotationSummary.focusRequests.some((f) => c.filePath === f.filePath && c.startLine <= f.line && c.endLine >= f.line));
    }
    ...
    const contextPrefixes = activeChunks.map((chunk) => mergeContexts(buildRetrievedContext(chunk, contextSourceChunks), getKeywordContext(chunk, keywordIndex)));
    ...
    const repoContextBlock = renderRepoContext(await buildRepoContext(manifests, contextSourceChunks, opts.projectRoot));
    if (repoContextBlock)
        context.repoContext = repoContextBlock;
    if (opts.target) {
    ...
    if (opts.dryRunConfig) {
        ...
        if (costEntries.length > 0) {
            console.log(kvTable(costEntries));
        }
        else {
    ...
    return runSwarm(activeChunks, context, {
        ...opts.swarmOptions,
