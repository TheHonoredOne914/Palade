...
export function jaccardSimilarity(a, b) {
    const getWords = (str) => new Set(str
        .toLowerCase()
        .split(/[^a-z0-9]+/)
        .filter((w) => w.length > 0));
    ...
    if (sameFile || bothFileless) {
        if (a.lineStart !== undefined && b.lineStart !== undefined) {
            ...
            if (jaccardSimilarity(a.title, b.title) > threshold)
                return true;
            if (jaccardSimilarity(a.description, b.description) > threshold)
                return true;
        }
    }
    return false;
...
export function mergeFindings(findings, opts = {}) {
    ...
    for (const indices of byFile.values()) {
        for (let a = 0; a < indices.length; a++) {
            ...
            for (let b = a + 1; b < indices.length; b++) {
                ...
                if (find(i) === find(j))
                    continue;
                if (shouldMerge(findings[i], findings[j], opts))
                    union(i, j);
            }
        }
    }
