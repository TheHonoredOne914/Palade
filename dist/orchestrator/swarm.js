...
import { scheduleBatches, estimateTotalTokens, ECONOMY_SOFT_TOKEN_CAP, ECONOMY_HARD_CHUNK_CAP, } from './scheduler.js';
...
export async function runSwarm(allChunks, context, options = {}, manifests) {
    ...
    const reviewChunks = manifests && !options.exhaustive
    ...
    const modeAgents = getAgentsForMode(context.mode, context.modeConfig?.agentOverrides, options.customAgents, options.agentCount);
    ...
    let expandedAgentProviders;
    if (options.providerShares) {
        expandedAgentProviders = expandProviderShares(options.providerShares, modeAgents.length, modeAgents.map((a) => a.name));
        updateAgentProviders(expandedAgentProviders);
    }
    let agents = modeAgents;
    ...
    if (options.economyMode) {
        ...
        if (builtInAgents.length <= 1) {
            console.warn(chalk.yellow('⚠ Economy mode requested but only 1 built-in agent active — falling back to standard mode.'));
        }
        if (builtInAgents.length > 1) {
            usingCombinedAnalyzer = true;
            const activeDomains = builtInAgents.map((a) => {
                const defaultSpec = DEFAULT_DOMAINS.find((d) => d.name === a.name);
                return (defaultSpec || { name: a.name, label: a.name, focus: 'General code review' });
            });
            ...
            if (options.providerShares && Object.keys(options.providerShares).length > 0) {
                const [pluralityProvider] = Object.entries(options.providerShares).sort((a, b) => b[1] - a[1])[0];
    ...
    const agentPromises = agents.map(async (agent) => {
        ...
        try {
            ...
            console.warn(chalk.yellow(`⚠ Error in agent start callback for ${agent.name}: ${err instanceof Error ? err.message : String(err)}`));
        }
        const allFindings = [];
        ...
        try {
            ...
            const agentSoftTokenLimit = agent instanceof CustomAgent ? options.softTokenLimit : softTokenLimit;
            const agentHardChunkLimit = agent instanceof CustomAgent ? options.hardChunkLimit : hardChunkLimit;
            ...
            const batchPromises = batches.map((batch, batchIdx) => limit(async () => {
                ...
                if (options.signal?.aborted || runAbort.signal.aborted)
                    controller.abort();
                let timeoutHandle;
    ...
    const mergedFindings = mergeFindings(memory.getAll(), {
    ...
    if (!options.noVerdict) {
        ...
        await Promise.allSettled(conflicts.map((conflict) => limit(async () => {
            ...
            if (verdict && verdict.is_conflict) {
                ...
                let savedNote = '';
                try {
                    const slug = await saveDecision(projectRoot, conflict, verdict, options.decisionsRetentionLimit);
                    savedNote = `\nSaved as: ${slug}.md`;
                }
                catch (err) {
                    console.warn(chalk.yellow(`⚠ Failed to save ADR decision: ${err instanceof Error ? err.message : String(err)}`));
                }
