 ...
 */
function hardTruncateChunk(chunk, hardChunkLimit) {
...
function splitChunk(chunk, hardChunkLimit) {
...
export function scheduleBatches(chunks, softTokenLimit = SOFT_TOKEN_LIMIT, hardChunkLimit = HARD_CHUNK_LIMIT) {
    if (chunks.length === 0)
        return [];
    // Split oversized chunks recursively to ensure all pieces are under hardChunkLimit
    function splitToLimit(chunk, depth = 0) {
        if (depth > 10) {
            ...
            console.warn(chalk.yellow(`⚠ scheduler: chunk ${chunk.id} still exceeded the ${hardChunkLimit}-token limit after ${depth} splits — hard-truncated from ${chunk.tokenCount} to ${truncated.tokenCount} tokens`));
            return [truncated];
        }
