...
export async function triageFiles(manifests, allChunks, options) {
    const budget = options?.maxReviewTokens ?? DEFAULT_MAX_REVIEW_TOKENS;
    const totalTokens = estimateTotalTokens(allChunks);
    if (totalTokens <= budget) {
        console.log(chalk.cyan(`  [triage] Project fits within token budget (${totalTokens.toLocaleString('en-US')}/${budget.toLocaleString('en-US')} tokens) — reviewing all files`));
        return allChunks;
    }
    console.log(chalk.red(`  [!] WARNING: Project token count (${totalTokens.toLocaleString('en-US')}) exceeds budget (${budget.toLocaleString('en-US')}). Some files will be silently dropped from the review.`));
    ...
    const compactManifest = manifests
        .map((m) => `${m.path} (${m.linesOfCode} lines, Churn: ${m.churnCount || 0}, Imports: ${m.importCount || 0})`)
        .join('\n');
    try {
        ...
        try {
            ...
            if (salvaged) {
                ...
                console.warn(chalk.yellow(`  [triage] LLM array JSON was malformed or truncated — salvaged ${salvaged.length} ranked path(s) from the partial response`));
            }
            else {
        ...
        if (validPaths.length > 0) {
            ...
            for (const rankedPath of validPaths) {
                ...
                if (matching.length === 0) {
                    const suffixMatches = allChunks.filter((c) => c.filePath.replace(/^\.?\/+/, '').endsWith('/' + clean));
                    const distinctFiles = new Set(suffixMatches.map((c) => c.filePath));
                    if (distinctFiles.size === 1)
                        matching = suffixMatches;
                }
                if (matching.length === 0) {
                    ...
                    const ciMatches = allChunks.filter((c) => c.filePath.replace(/^\.?\/+/, '').toLowerCase() === cleanLower);
            ...
            if (selected.length > 0) {
                ...
                console.log(chalk.cyan(`  [triage] Selected ${selected.length} chunks (${tokensUsed.toLocaleString('en-US')} tokens) from ${selected.length > 0 ? new Set(selected.map((c) => c.filePath)).size : 0} files`));
                return selected;
            }
        }
    }
...
export function scoreManifestForReview(m) {
    let score = 0;
    const p = m.path.toLowerCase();
    if (/auth|login|session|token|password|secret|key/.test(p))
        score += 10;
    if (/api|route|handler|controller|endpoint/.test(p))
        score += 8;
    if (/service|repository|dao|store/.test(p))
        score += 6;
    if (/middleware|interceptor|guard|acl|permission|policy|tenant|scope|rbac|access/.test(p)) {
        score += 8;
    }
    if (/runtime|vm|sandbox|exec|spawn|crypto|eval/.test(p))
        score += 10;
    if (/payment|billing|stripe|webhook/.test(p))
        score += 9;
    if (/util|helper|common|shared/.test(p))
        score += 4;
    if (/config|env|settings/.test(p))
        score += 7;
    if (/test|spec|mock|fixture/.test(p))
        score -= 5;
    const boundaryTypeFile = /(auth|user|payment|session|security|permission|policy|rbac).*(type|interface|dto)/.test(p) ||
        /(type|interface|dto).*(auth|user|payment|session|security|permission|policy|rbac)/.test(p);
    if (/type|interface|dto/.test(p) && !boundaryTypeFile)
        score -= 2;
    score += Math.min(m.linesOfCode / 50, 5);
    return score;
}
function heuristicSelect(manifests, allChunks, budget) {
    ...
    console.log(chalk.cyan(`  [triage] Heuristic selected ${selected.length} chunks (${tokensUsed.toLocaleString('en-US')} tokens) from ${new Set(selected.map((c) => c.filePath)).size} files`));
    return selected;
}
