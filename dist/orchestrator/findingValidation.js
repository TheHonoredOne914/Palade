...
function getMatchingChunkAndClamp(finding, chunk) {
    ...
    if (lineStart < chunk.startLine || lineStart > chunk.endLine)
        return null;
...
function fingerprintFor(finding) {
    ...
    const lineBucket = typeof finding.lineStart === 'number' && Number.isInteger(finding.lineStart)
...
export function validateAndFingerprintFindings(findings, chunks) {
    ...
    for (const finding of findings) {
        const tags = Array.isArray(finding.tags)
            ? finding.tags.filter((tag) => typeof tag === 'string')
            : [];
        if (!finding.filePath) {
            validated.push({
                ...finding,
        ...
        const lineStart = finding.lineStart === undefined
            ? undefined
            : Number.isInteger(finding.lineStart)
                ? finding.lineStart
                : Math.round(finding.lineStart);
        const lineEnd = finding.lineEnd === undefined
            ? undefined
            : Number.isInteger(finding.lineEnd)
                ? finding.lineEnd
                : Math.round(finding.lineEnd);
        let clampedFinding = null;
        ...
        for (const chunk of candidateChunks) {
            ...
            const fullyContains = lineStart === undefined ||
                lineEnd === undefined ||
