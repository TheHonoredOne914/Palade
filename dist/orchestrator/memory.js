...
import { SEVERITY_RANK, isNearMatch, jaccardSimilarity, NEAR_MATCH_SAME_AGENT_THRESHOLD, NEAR_MATCH_CROSS_AGENT_THRESHOLD, } from './merger.js';
// Derived from merger.ts's SEVERITY_RANK (rather than a separately maintained
// array) so the two orderings can't drift apart.
const SEVERITY_ORDER = Object.keys(SEVERITY_RANK).sort((a, b) => SEVERITY_RANK[a] - SEVERITY_RANK[b]);
...
export class AgentMemory {
    ...
    crossReference() {
        ...
        const findingsByFileNoLine = new Map();
        ...
        for (const [filePath, fileFindings] of findingsByFile) {
            ...
            for (let i = 0; i < n; i++) {
                for (let j = i + 1; j < n; j++) {
                    if (isNearMatch(fileFindings[i], fileFindings[j]))
                        union(i, j);
                }
            }
            ...
            for (const clusterFindings of clusters.values()) {
                ...
                const lineRange = [lines[0], lines[lines.length - 1]];
                const locLabel = lines[0] === lines[lines.length - 1]
                    ? `${filePath}:${lines[0]}`
        ...
        for (const [filePath, fileFindings] of findingsByFileNoLine) {
            ...
            for (let i = 0; i < n; i++) {
                for (let j = i + 1; j < n; j++) {
                    ...
                    const threshold = a.agentName === b.agentName
                        ? NEAR_MATCH_SAME_AGENT_THRESHOLD
                        : NEAR_MATCH_CROSS_AGENT_THRESHOLD;
                    if (jaccardSimilarity(a.title, b.title) > threshold ||
        ...
        const rangesOverlap = (a, b, symA, symB) => {
            if (a && b)
        ...
        const agentSetsEqual = (a, b) => a.size === b.size && [...a].every((v) => b.has(v));
        const crossFindings = [];
        const accepted = [];
        for (const candidate of rawCrossFindings) {
            const isDuplicate = accepted.some((existing) => agentSetsEqual(existing.agents, candidate.agents) &&
                setsOverlap(existing.filePaths, candidate.filePaths) &&
                rangesOverlap(existing.lineRange, candidate.lineRange, existing.symbolKey, candidate.symbolKey));
            if (isDuplicate)
                continue;
