 ...
 * signal.
 */
export function isDailyLimitError(body) {
    try {
...
export function tagQuotaError(err) {
    ;
    err[QUOTA_ERROR_TAG] = true;
    return err;
}
/**
 ...
 */
export function rateLimitedMessage(providerLabel, body) {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        ...
        try {
            ...
            if ([500, 502, 503, 504].includes(res.status) && attempt < retries) {
                ...
                await sleep(Math.min(BASE_DELAY_MS * 2 ** attempt, MAX_DELAY_MS) * (0.5 + Math.random() * 0.5), externalSignal);
                continue;
            }
            return res;
        }
...
export function shouldRetryEmptyContent(content, outputTokens, attempt, maxAttempts = 2) {
    return content.trim().length === 0 && outputTokens > 0 && attempt < maxAttempts;
}
export function nextRetryMaxTokens(maxTokens, ceiling = MAX_RETRY_TOKENS) {
