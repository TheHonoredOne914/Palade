import { fetchWithRetry, createLimiter, isDailyLimitError, shouldRetryEmptyContent, nextRetryMaxTokens, DEFAULT_DEADLINE_MS, tagQuotaError, rateLimitedMessage, } from './base.js';
import { AuthError } from '../errors/types.js';
const defaultExtractContent = (message) => message?.content ?? '';
export class OpenAICompatibleProvider {
    ...
    deadGeneric = false;
    constructor(cfg, apiKey, model, maxConcurrency, baseUrl, deadlineMs = DEFAULT_DEADLINE_MS) {
        this.name = cfg.name;
    ...
    async doComplete(req, maxTokens, attempt, deadline, retryCeiling) {
        ...
        if (!res.ok) {
            ...
            if (res.status === 401 || res.status === 403) {
                throw new AuthError(`${this.label} error ${res.status}: ${body.slice(0, 200)}`, res.status, this.name);
            }
        ...
        if (!Array.isArray(data.choices)) {
            throw new Error(`${this.label} error: malformed response body — missing or invalid "choices" array`);
        }
