    ...
    async complete(req) {
        ...
        if (!candidate && (provider.isDead?.() ?? false)) {
            const deadErr = provider.isDeadFromAuth?.()
                ? new AuthError(`${provider.name} is already marked dead from an auth error earlier this session.`, 401, provider.name)
                : tagQuotaError(new Error(`${provider.name} is already marked dead (quota exhausted) earlier this session.`));
            deadErr[PROVIDER_POOL_SOURCE] = provider;
            throw deadErr;
        }
        try {
            ...
            if (err instanceof Error) {
                ;
                err[PROVIDER_POOL_SOURCE] = provider;
            }
            throw err;
        }
    }
