...
import { annotateComplexity, buildChunkContext, buildSystemPrompt, computeMaxTokens, completeAndParseFindings, unparsableResponseFinding, verifyCriticalHighFindings, } from './base.js';
...
import { TEST_INTELLIGENCE_WARNING, TEST_INTELLIGENCE_FOCUS, } from './specialist/testIntelligence.js';
...
function buildCombinedSystemPrompt(domains) {
...
export class CombinedAnalyzer {
    ...
    async analyze(chunks, context, signal) {
        try {
            ...
            const { findings: rawFindings, response } = await completeAndParseFindings(provider, { systemPrompt, userPrompt, maxTokens, signal }, this.name, true);
            const findings = attributeFindings(rawFindings, this.domains, response.provider, response.model);
            ...
            if (droppedCount > 0) {
                findings.push(...unparsableResponseFinding(this.name, `dropped ${droppedCount} finding(s) with an agentName that didn't match any active domain`));
            }
...
// 'architecture').
const AGENT_NAME_ALIASES = { architect: 'architecture' };
...
export function attributeFindings(findings, domains, provider, model) {
    ...
    for (const f of findings) {
        ...
        if (!validNames.has(f.agentName)) {
            ...
            const canonical = Array.from(validNames).find((n) => normalizeAgentName(n) === norm) ??
                (aliased && validNames.has(aliased) ? aliased : undefined);
            if (canonical)
                f.agentName = canonical;
        }
        if (!validNames.has(f.agentName)) {
            console.warn(chalk.yellow(`⚠ combined: dropped finding "${f.title}" with unrecognized agentName "${f.agentName}"`));
            continue;
        }
        f.provider = provider;
        f.model = model;
