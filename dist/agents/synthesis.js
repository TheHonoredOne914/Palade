...
function parseSynthesisResponse(raw) {
    ...
    const priorityFixes = Array.isArray(obj.priorityFixes)
        ? obj.priorityFixes
            ...
            const hours = typeof f.estimatedHours === 'number'
                ? f.estimatedHours
            ...
            return {
                ...
                affectedFiles: Array.isArray(f.affectedFiles)
                    ? f.affectedFiles.filter((x) => typeof x === 'string')
                    : [],
            };
        })
        : [];
    const crossCuttingObservations = Array.isArray(obj.crossCuttingObservations)
        ? obj.crossCuttingObservations.filter((o) => typeof o === 'string')
        : [];
    ...
    const rawDebt = obj.debtEstimate;
    const debtEstimate = {
        critical: 0,
        high: 0,
        medium: 0,
 ...
 */
function selectFindingsForSynthesis(sorted, cap) {
...
function computeDebtCounts(findings) {
 ...
 */
function computeDebtHours(findings) {
    const sums = { critical: 0, high: 0, medium: 0, low: 0, total: 0 };
    for (const f of findings) {
        const hours = typeof f.estimatedHours === 'number' && Number.isFinite(f.estimatedHours)
            ? f.estimatedHours
            : 0;
        if (f.severity === 'critical')
            sums.critical += hours;
 ...
 */
function computeGhostHours(findings) {
    const sums = { critical: 0, high: 0, medium: 0, low: 0, total: 0 };
    for (const f of findings) {
        const hours = typeof f.hoursWasted === 'number' && Number.isFinite(f.hoursWasted) ? f.hoursWasted : 0;
        if (f.severity === 'critical')
            sums.critical += hours;
        else if (f.severity === 'high')
            sums.high += hours;
...
export async function synthesize(allFindings, crossAgentFindings, context, options = {}) {
    const { maxSynthesisFindings = 50, synthesisTimeoutMs = 180_000, signal, severityWeights, } = options;
    ...
    const debtCounts = context.mode === 'debt'
    ...
    try {
        ...
        const sorted = [...allFindings].sort((a, b) => penaltyFor(b, severityWeights) - penaltyFor(a, severityWeights));
