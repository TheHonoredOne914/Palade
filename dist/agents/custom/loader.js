...
export async function loadCustomAgents(projectRoot) {
    ...
    try {
        ...
        throw new PaladeConfigError(`Failed to import ${AGENTS_FILE}: ${msg}`, 'agents', `Fix the error in ${AGENTS_FILE} or delete the file to skip custom agents.`);
    }
    ...
    if (!Array.isArray(raw)) {
        throw new PaladeConfigError(`${AGENTS_FILE} must export an array of agent definitions (found ${typeof raw}).`, 'agents', `Ensure ${AGENTS_FILE} has \`export default [ { name, domain, systemPrompt }, ... ]\`.`);
    }
    const agents = [];
    ...
    for (let i = 0; i < raw.length; i++) {
        ...
        if (result.success) {
            ...
            if (seenNames.has(result.data.name)) {
                throw new PaladeConfigError(`${AGENTS_FILE} entry at index ${i} duplicates agent name '${result.data.name}'.`, 'agents', `Give each custom agent in ${AGENTS_FILE} a unique name.`);
            }
            ...
            const issues = result.error.issues
                ...
                .join('; ');
            throw new PaladeConfigError(`${AGENTS_FILE} entry at index ${i} is invalid: ${issues}`, 'agents', `Edit ${AGENTS_FILE} entry ${i} to satisfy the schema (name, domain, systemPrompt required; name must not collide with a built-in).`);
        }
    }
    return agents;
}
