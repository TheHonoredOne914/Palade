...
export const BUILTIN_NAMES = Array.from(BUILTIN_AGENTS.keys());
/**
 ...
 */
export const AGENT_REGISTRY = Array.from(BUILTIN_AGENTS.values());
export function getAgentsForMode(mode, agentOverrides, customAgentDefs = [], agentCount) {
    ...
    const allAgents = new Map([...BUILTIN_AGENTS, ...customAgents]);
    ...
    const effectiveOverrides = agentOverrides && agentOverrides.length > 0
        ? agentOverrides
        : mode === 'ghost'
    ...
    if (effectiveOverrides && effectiveOverrides.length > 0) {
        ...
        if (agents.length === 0) {
            throw new PaladeConfigError(`agentOverrides contains no recognized agent names: ${effectiveOverrides.join(', ')}`, 'agentOverrides', `Available agents: ${[...allAgents.keys()].join(', ')}`);
        }
        ...
        if (unmatched.length > 0) {
            throw new PaladeConfigError(`agentOverrides contains unrecognized agent names: ${unmatched.join(', ')}`, 'agentOverrides', `Available agents: ${[...allAgents.keys()].join(', ')}`);
        }
        ...
        for (const customAgent of customAgents.values()) {
            if (!overriddenNames.has(customAgent.name))
    ...
    const builtIns = agentCount && agentCount > 0 ? AGENT_REGISTRY.slice(0, agentCount) : AGENT_REGISTRY;
