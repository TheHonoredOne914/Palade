...
function resolveImport(fromFile, importPath, projectRoot) {
    ...
    if (pyRel && !normalizedImport.includes('/')) {
        ...
        if (up > dirParts.length)
            return projectRoot; // refuse traversal above root
        dirParts.length -= up;
...
export async function traceDependencies(filePath, projectRoot, depth = 1) {
    ...
    async function trace(currentPath, currentDepth) {
        if (currentDepth > depth)
            return;
        const normalizedCurrent = normalizePath(currentPath);
        if ((visited.get(normalizedCurrent) ?? Infinity) <= currentDepth)
            return;
