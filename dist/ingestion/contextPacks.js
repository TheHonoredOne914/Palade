...
function withoutExtension(path) {
    return path.replace(/\.(tsx?|jsx?|mts|cts|mjs|cjs|py|go|rs|java|cs|cpp|cc|c|h|hpp|rb|php|swift|kt|dart)$/, '');
}
...
function identifierTerms(content) {
    const terms = new Set();
...
const identifierTermsCache = new WeakMap();
function getIdentifierTerms(chunk) {
...
function expectedTestBases(filePath) {
...
function scoreRelatedChunk(subject, candidate, subjectImports, subjectTerms, subjectTestBases, subjectBase) {
    ...
    for (const spec of subjectImports) {
        ...
        if (resolved && candidateBase === withoutExtension(resolved))
            score += 10;
    }
    for (const testBase of subjectTestBases) {
        if (candidateBase === testBase)
            score += 8;
    }
    if (subjectBase && candidate.content.includes(subjectBase))
        score += 4;
...
export function buildRetrievedContext(subject, allChunks) {
