...
export function hardSplitBudget(hardChunkLimit) {
...
export function splitLargeChunk(chunk, hardChunkLimit = MAX_TOKENS) {
    const effectiveLimit = hardSplitBudget(hardChunkLimit);
    if (chunk.tokenCount <= effectiveLimit)
        return [chunk];
...
function calculateComplexityForNodes(nodes) {
    let complexity = 1;
    const visit = (n) => {
        if (isComplexityNode(n))
            complexity++;
...
function getTopLevelSymbolName(node) {
...
function chunkByAST(content, filePath, language) {
...
function isRegexStartAllowed(lastSignificantChar, lastWord) {
...
function chunkByBrackets(content, filePath, language) {
...
async function chunkOneFile(manifest) {
    let content;
    try {
    ...
    if (isAstLanguage && !exceedsParseLimits) {
        try {
            ...
            console.warn(`[chunker] AST parsing failed for ${manifest.path} (${err instanceof Error ? err.message : String(err)}) — falling back to line-based chunking.`);
            chunks = chunkByBrackets(content, manifest.path, manifest.language).flatMap(splitLargeChunk);
        }
    }
    else {
        if (isAstLanguage && exceedsParseLimits) {
            console.warn(`[chunker] File ${manifest.path} (${lineCount} lines, ${byteSize} bytes) exceeds the ` +
    ...
    if (chunks.length > MAX_CHUNKS_PER_FILE) {
        ...
        console.error(`[chunker] TRUNCATED: ${manifest.path} produced ${chunks.length} chunks, exceeding the ` +
        ...
        if (lastChunk)
            lastChunk.truncated = true;
    }
    return chunks;
}
export async function chunkFiles(manifests) {
    ...
    const perFileChunks = await Promise.all(manifests.map((manifest) => limit(() => chunkOneFile(manifest))));
