...
function resolveReexport(specifier, fromFile, manifestPaths) {
    ...
    for (const ext of extensions) {
        if (manifestPaths.has(resolved + ext))
            return resolved + ext;
    }
    ...
    if (jsStripped !== resolved) {
        for (const ext of ['.ts', '.tsx']) {
            if (manifestPaths.has(jsStripped + ext))
                return jsStripped + ext;
        }
    }
    for (const ext of ['.ts', '.tsx', '.js', '.jsx']) {
        if (manifestPaths.has(`${resolved}/index${ext}`))
            return `${resolved}/index${ext}`;
    }
    return null;
}
function buildPublicApi(pkg, manifestPaths, chunksByFile) {
    if (!pkg) {
    ...
    for (const field of ['main', 'module', 'types', 'exports', 'bin']) {
        if (pkg[field] !== undefined)
            collectStrings(pkg[field], rawEntries);
    }
        ...
        for (const chunk of chunksByFile.get(file) ?? []) {
            for (const m of chunk.content.matchAll(REEXPORT_RE)) {
                ...
                if (target && !reached.has(target)) {
                    reached.add(target);
                    queue.push(target);
                    if (reached.size >= MAX_PUBLIC_API_BFS)
                        break;
                }
            }
        }
...
export async function buildRepoContext(manifests, chunks, projectRoot) {
    let pkg;
    try {
    ...
    for (const m of manifests) {
        for (const importer of m.importers ?? []) {
            if (!adjacency.has(importer))
                adjacency.set(importer, new Set());
            ...
            for (const line of chunk.content.split('\n')) {
                ...
                if (match) {
                    ...
                    moduleGlobals.push({
                        ...
                        hasCleanup: fullContent.includes(`${name}.delete(`) || fullContent.includes(`${name}.clear(`),
                    });
                }
            }
        }
        if (moduleGlobals.length >= MAX_MODULE_GLOBALS)
            break;
    }
    return {
        dependencyCycles,
...
function renderCappedList(items, cap) {
...
export function renderRepoContext(ctx) {
    const sections = [];
    ...
    if (ctx.publicApiFiles.length > 0) {
        sections.push(`PUBLIC API FILES (exports here are consumed by library users — do not report as dead code):\n${renderCappedList(ctx.publicApiFiles, MAX_PUBLIC_API_RENDER)}`);
    }
    ...
    if (ctx.buildTimeFiles.length > 0) {
        sections.push(`BUILD-TIME FILES (not runtime-reachable — deprioritize injection/XSS findings here):\n${ctx.buildTimeFiles.map((i) => `  - ${i}`).join('\n')}`);
    }
    ...
    if (ctx.moduleGlobals.length > 0) {
        const items = ctx.moduleGlobals.map((g) => `${g.file}: ${g.name} (${g.kind}, ${g.hasCleanup ? 'has delete/clear' : 'no delete/clear found'})`);
        sections.push(`MODULE-LEVEL COLLECTIONS (check for unbounded growth; entries without cleanup are leak candidates):\n${renderCappedList(items, MAX_MODULE_GLOBALS)}`);
    }
    // Only PROJECT TYPE is unconditional; if nothing else has content, treat as empty.
    const hasRealContent = ctx.dependencyCycles.length > 0 ||
