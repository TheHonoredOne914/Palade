...
export function parseFile(absolutePath, isHashComment = false) {
    return parseFileAsync(absolutePath, isHashComment);
}
async function parseFileAsync(absolutePath, isHashComment) {
    let content;
    try {
...
export function buildAnnotationSummary(manifests, chunks) {
...
export function applyLineIgnores(findings, ignoredLines) {
    if (ignoredLines.length === 0)
        return findings;
    const norm = (p) => p.replace(/^\.?\/+/, '');
    return findings.filter((f) => {
        if (!f.filePath || f.lineStart === undefined)
            return true;
        return !ignoredLines.some((il) => norm(il.filePath) === norm(f.filePath) &&
