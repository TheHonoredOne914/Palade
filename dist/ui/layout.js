...
export function sectionBox(title, content) {
    return drawBox(content, title);
}
export function formatDriftAlert(filePath, findings) {
    const lines = [];
...
export function sparkline(values, width = 20) {
    ...
    const chars = values.slice(-width).map((v) => {
        ...
        if (v >= 80)
            return chalk.hex('#10B981')(block);
        if (v >= 60)
            return chalk.hex('#F59E0B')(block);
        if (v >= 40)
            return chalk.hex('#F97316')(block);
...
export function scoreGrade(score) {
    ...
    if (score >= SCORE_THRESHOLDS.good)
        return 'A';
    if (score >= SCORE_THRESHOLDS.good - 5)
        return 'B+';
    if (score >= SCORE_THRESHOLDS.warning + 10)
        return 'B';
