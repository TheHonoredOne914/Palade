 ...
 */
async function askQuestionOrClose(query) {
...
export async function askList(query, choices) {
...
export async function askCheckbox(query, choices) {
    // Intentional defense-in-depth (uicli-007) — see askQuestion's guard above.
    if (process.env.PALADE_TUI) {
        console.warn('[prompt] Readline blocked under TUI');
        return [];
    ...
    while (true) {
        const answer = await askQuestionOrClose(`Select options by number (e.g. "1 2 4", or "all", or "none"): `);
