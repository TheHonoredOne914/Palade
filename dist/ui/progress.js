...
export function createLiveProgress() {
    return {
        ...
        agentDone(name, findings, ms, err) {
            ...
            const countStr = findings > 0 ? theme.warning(countRaw.padEnd(20)) : theme.success(countRaw.padEnd(20));
            console.log(`  ${theme.success('✓')} ${theme.dim(name.padEnd(22))} ${countStr} ${theme.dim(time)}`);
        },
