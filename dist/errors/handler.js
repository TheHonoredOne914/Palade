...
import { PaladeConfigError, NoProvidersError, OllamaNotRunningError, TargetNotFoundError, SwarmTimeoutError, WorkspaceTooLargeError, CliExitError, } from './types.js';
import { sanitizeForLog } from '../utils/sanitize.js';
import { AllProvidersExhaustedError } from '../providers/router.js';
export function formatErrorMessages(err) {
    if (err instanceof PaladeConfigError) {
        const lines = [chalk.red(`Configuration error at ${err.field}:`), `  ${err.message}`];
        if (err.suggestion)
            lines.push(chalk.dim(`  Hint: ${err.suggestion}`));
        return lines;
    }
    ...
    if (err instanceof Error) {
        ...
        if (debug) {
            ...
            const { message: _message, stack: _stack, ...extra } = err;
