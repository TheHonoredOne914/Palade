...
process.emit = function (name, ...args) {
    ...
    if (name === 'warning' && typeof data === 'object' && data !== null) {
        const w = data;
        if (w.code === 'MODULE_TYPELESS_PACKAGE_JSON' ||
...
const hasCommand = commandScanArgs.some((a) => !a.startsWith('-') && a.length > 0) ||
...
async function runClassicCLI() {
    ...
    program
        ...
        .option(VALUE_FLAG_STRINGS.mode, 'Review mode: standard|security|onboard|debt|ghost', 'standard')
        ...
        .option('-e, --exhaustive', 'Exhaustive mode: skip triage and analyze the entire project (maximum issues)')
        ...
        try {
            await reviewCommand(pathArg, opts);
        }
        catch (err) {
    ...
    program
        ...
        .option(VALUE_FLAG_STRINGS.set, 'Set a config value (repeatable)', (val, prev) => prev.concat([val]), [])
