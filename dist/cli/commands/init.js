...
export async function initCommand(opts) {
    ...
    const existingFiles = [];
    if (configExists)
        existingFiles.push('.palade/palade.config.ts');
    if (targetsExists)
        existingFiles.push('.palade/palade.targets.ts');
    if (agentsExists)
        existingFiles.push('.palade/palade.agents.ts');
    if (ignoreExists)
        existingFiles.push('.palade/ignore');
    if (existingFiles.length > 0 && !opts?.yes) {
        if (!process.stdin.isTTY || opts?.tui) {
            ...
            const proceed = await askConfirm(`${existingFiles.length} config file(s) already exist. Create missing files and update .gitignore?`);
            if (!proceed) {
    ...
    try {
        ...
        const configuredProviders = Object.entries(config.providers ?? {}).filter(([, p]) => p.apiKey);
