...
export const targetsCommand = new Command('targets').description('Manage review targets (subsystems)');
function throwIfAborted(signal) {
    if (signal?.aborted) {
        throw new CliExitError(1);
    }
}
export async function runTargetsSearch(query, signal) {
...
export async function runTargetsAdd(pkg, signal) {
...
// ============================================================================
export async function runTargetsGenerate(query, signal) {
    ...
    appendTargetToFile(process.cwd(), target);
    console.log(theme.success(`Target "${target.name}" generated and installed into .palade/palade.targets.ts
  Run ${chalk.cyan(`palade review --target ${target.name}`)} to review it.`));
}
export async function runTargetsList(signal) {
    ...
    for (const t of targets) {
        console.log(`  ${theme.accent(t.name.padEnd(16))} ${theme.dim(Array.isArray(t.entry) ? t.entry.join(', ') : t.entry)}`);
        if (t.description) {
