...
export async function scoreCommand(opts) {
    ...
    try {
        ...
        console.log();
        console.log(sectionBox('Score History', [
            kvTable([
                [
                    'Current score:',
                    scoreTheme(latest.score)(`${latest.score}/100  ${formatDelta(latest.delta)}  Grade: ${scoreGrade(latest.score)}`),
                ],
        ...
        const lines = [];
        lines.push(`  ${theme.primaryBold('Date'.padEnd(18))} ${theme.primaryBold('Score'.padEnd(12))} ${theme.primaryBold('Delta')}`);
        ...
        if (err instanceof CliExitError)
            throw err;
        console.error(theme.error(`Score lookup failed: ${err.message}`));
        throw new CliExitError(1);
    }
}
