...
export async function reviewCommand(pathArg, opts) {
    if (opts.target && opts.allTargets) {
        console.error(chalk.red("  Cannot use --target and --all-targets together — --all-targets would silently override --target's scope. Pick one."));
    ...
    if (opts.format) {
        ...
        if (invalidFormats.length > 0) {
            console.error(chalk.red(`  Invalid --format value${invalidFormats.length > 1 ? 's' : ''}: ${invalidFormats.join(', ')}. Valid options: ${VALID_REPORT_FORMATS.join(', ')}`));
            throw new CliExitError(1);
        }
    }
    if (!pathArg &&
    ...
    if (symbolFilter) {
        ...
        if (!Number.isFinite(depth)) {
            ...
            console.log(theme.warning(`  Invalid --depth value "${String(opts.depth)}" — defaulting to depth 1.`));
            depth = 1;
        }
        if (depth > 0) {
            ...
            if (symbolChunks.length > 1) {
                console.log(theme.dim(`  + ${symbolChunks.length - 1} dependency file(s) traced (depth ${depth})`));
            }
            if (deps.length > 10) {
    ...
    if (opts.pick) {
        const allManifests = await import('../../ingestion/walker.js').then((m) => m.walkProject(projectRoot, { projectRoot }));
    ...
    if (opts.target) {
        ...
        if (!match) {
            console.error(chalk.red(`Target "${opts.target}" not found. Available: ${allTargets.map((t) => t.name).join(', ') || '(none)'}`));
            throw new CliExitError(1);
        }
        resolvedTarget = {
            definition: match,
        ...
        if (match.scope) {
            if (match.scope.dirs?.length)
                scope.dirs = [...(scope.dirs ?? []), ...match.scope.dirs];
            if (match.scope.files?.length)
                scope.files = [...(scope.files ?? []), ...match.scope.files];
            if (match.scope.globs?.length)
                scope.globs = [...(scope.globs ?? []), ...match.scope.globs];
    ...
    if (opts.allTargets) {
        if (allTargets.length === 0) {
            ...
            for (const t of allTargets) {
                for (const p of resolveTargetPaths(t, projectRoot))
                    unionPaths.add(p);
            }
    ...
    if (!opts.quiet) {
        ...
        const rows = [
            ...
            [
                'Scope:',
                theme.dim(opts.target ? `target: ${opts.target}` : opts.dir ? `dir: ${opts.dir}` : 'full codebase'),
            ],
    ...
    try {
        swarmResult = await runPipeline({
            ...
            swarmOptions: {
                onAgentStart: (name) => {
                    if (opts.tui)
                        console.log(theme.dim(`  Starting ${name} agent...`));
                    progress?.agentStart(name);
                },
                onAgentBatchComplete: (name, current, total, findings) => {
                    progress?.agentBatchDone(name, current, total, findings);
                },
                onAgentComplete: (name, findings, durationMs, error) => {
                    if (opts.tui) {
                        if (error) {
                            console.log(theme.error(`  ✖ ${name} agent failed: ${error.message}`));
                        }
                        else {
                            console.log(theme.success(`  ✓ ${name} agent finished in ${(durationMs / 1000).toFixed(1)}s (${findings} findings)`));
                        }
                    }
                ...
                onSynthesisComplete: (durationMs) => {
                    if (opts.tui) {
                        console.log(theme.success(`  ✓ Synthesis complete in ${(durationMs / 1000).toFixed(1)}s`));
                    }
        ...
        if (Array.isArray(partialFindings) && partialFindings.length > 0) {
            console.warn(chalk.yellow(`  ⚠ ${partialFindings.length} finding(s) were collected before the fatal error and were discarded.`));
        }
    ...
    const scoreResult = calculateScore(swarmResult.findings, swarmResult.crossAgentFindings, previousScore, {
        severityWeights: config.score.severityWeights,
    ...
    if (config.score.badge) {
        console.log(`  ${theme.dim('→ Badge')}     ${chalk.cyan(config.score.badgePath)} ${theme.success('updated')}`);
    }
    console.log();
    // 15. Open browser
