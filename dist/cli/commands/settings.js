...
import { PROVIDERS, saveApiKey, setNestedValue } from '../../config/apiKey.js';
...
export async function settingsCommand(opts) {
...
async function interactiveSettings(projectRoot) {
    ...
    if (!process.stdin.isTTY) {
        console.error(chalk.red('  Interactive settings require a terminal (no stdin TTY detected). ' +
...
async function showCurrentConfig(projectRoot) {
    ...
    try {
        ...
        for (const prov of PROVIDERS) {
            const p = config.providers[prov.id];
            if (p) {
        ...
        console.log(`    Agents:     ${chalk.cyan(String(config.swarm?.agentCount ?? DEFAULT_CONFIG.swarm?.agentCount ?? 8))}`);
...
async function initConfig(projectRoot) {
...
async function applySets(projectRoot, sets) {
    const configPath = join(projectRoot, '.palade', 'palade.config.ts');
    let configContent;
    try {
