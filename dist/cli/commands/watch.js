...
class WatchController {
    ...
    isProcessing = false;
    accumulatedFindings = new Map();
    MAX_ACCUMULATED_FILES = 200;
    sweepQueue = [];
    urgentQueue = [];
    urgentSet = new Set();
    sweepSet = new Set();
    ...
    resolveDone;
    donePromise;
    boundOnExit = this.onExit.bind(this);
    boundOnSigint = this.onSigint.bind(this);
    constructor(projectRoot, debounceMs, isContinuous, config, watchAgents) {
    ...
    async start() {
        if (this.isContinuous) {
            console.log(theme.dim(`  Continuous background sweep enabled. Resolving initial file list...`));
            try {
    ...
    async processNext() {
        ...
        try {
            ...
            if (isUrgent) {
                ...
                if (nextFile)
                    this.urgentSet.delete(nextFile);
    ...
    async analyzeFile(filePath, signal) {
        ...
        try {
            ...
            const { softTokenLimit, hardChunkLimit } = applyEconomyLimits(usingCombinedAnalyzer, this.config.swarm.softTokenLimit, this.config.swarm.hardChunkLimit);
            ...
            if (finalFindings.length > 0) {
                ...
                if (this.accumulatedFindings.size > this.MAX_ACCUMULATED_FILES) {
                    const keysToDelete = [...this.accumulatedFindings.keys()].slice(0, this.accumulatedFindings.size - this.MAX_ACCUMULATED_FILES);
                    for (const key of keysToDelete)
                        this.accumulatedFindings.delete(key);
                }
...
export async function watchCommand(opts) {
    ...
    if (!(sensitivity in DEBOUNCE_MS)) {
        console.error(chalk.red(`Invalid --sensitivity value "${sensitivity}". Valid values: ${Object.keys(DEBOUNCE_MS).join(', ')}`));
    ...
    try {
        ...
        console.error(chalk.red(`Failed to initialise: ${err.message}`));
        throw new CliExitError(1);
    }
    ...
    console.log(theme.accent(`  palade watch started. Watching for changes... (${sensitivity} sensitivity)`));
