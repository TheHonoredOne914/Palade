...
export async function launchTUI() {
    ...
    const hasConfigFile = existsSync(join(process.cwd(), 'palade.config.ts')) ||
    ...
    const { waitUntilExit } = render(_jsx(App, { config: config, providerStatus: providerStatus, projectRoot: process.cwd(), version: pkg.version, configError: configError, noProvider: noProvider }), {
        exitOnCtrlC: false,
        patchConsole: false,
    });
    await waitUntilExit();
