...
export class CerebrasProvider implements IProvider {
  ...
  async complete(req: CompletionRequest): Promise<CompletionResponse> {
    ...
    if (this.deadGeneric) {
      throw new Error('Cerebras provider marked dead for this session (see earlier fatal error)')
    }
    return this.limiter(() =>
  ...
  private async doComplete(
    ...
    if (res.status === 429) {
      ...
      if (isDailyLimitError(body)) {
        ...
        throw tagQuotaError(new Error(`Cerebras daily limit exceeded. ${body.slice(0, 200)}`))
      }
      throw new Error(rateLimitedMessage('Cerebras', body))
    }
