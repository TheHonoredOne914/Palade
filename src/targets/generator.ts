...
export async function generateTarget(query: string, projectRoot: string): Promise<TargetDefinition | null> {
  ...
  const maxFiles = 2500
  const fileList = manifests.slice(0, maxFiles).map(m => m.path).join('\n')
  ...
  try {
    ...
    console.error(`[generator] Failed to call primary provider: ${err instanceof Error ? err.message : String(err)}`)
    return null
  }
  ...
  try {
    ...
    return {
      ...
      description: typeof parsed.description === 'string' ? parsed.description : 'AI generated target',
      entry: Array.isArray(parsed.entry) ? parsed.entry as string[] : typeof parsed.entry === 'string' ? [parsed.entry] : [],
