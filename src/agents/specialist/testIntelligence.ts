...
// (agents-001).
export const TEST_INTELLIGENCE_WARNING = `CRITICAL CONTEXT WARNING: You are reviewing PARTIAL chunks of a codebase. You do not have the full test suite or coverage maps. Only flag LOCALLY obvious testing gaps (e.g. a complex conditional branch that clearly lacks a fallback, or highly coupled I/O logic that is inherently untestable). Do NOT claim "zero test coverage" for a module just because tests aren't in the current chunk.`

export const TEST_INTELLIGENCE_FOCUS = `Additional test intelligence focus:
