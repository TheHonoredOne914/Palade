...
export async function decisionsCommand(
  ...
  if (act === 'list') {
    ...
    for (const file of mdFiles) {
      ...
      const dateLine = lines.find(l => l.startsWith('**Date:**'))?.replace('**Date:**', '').trim()
      const fileLine = lines.find(l => l.startsWith('**File:**'))?.replace('**File:**', '').trim()
    ...
    for (const file of mdFiles) {
      ...
      if (ageDays > days) {
        found = true
        console.log(`  ${theme.warning(file.replace('.md', ''))} ${theme.dim(`(${Math.floor(ageDays)} days old)`)}`)
      }
    }
