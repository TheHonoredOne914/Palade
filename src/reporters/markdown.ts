...
function renderCategoryScoresTable(categories: { category: ScoreCategory; score: number; findingCount: number }[]): string {
...
function renderPriorityFixes(fixes: { rank: number; title: string; rationale: string; estimatedHours: number; affectedFiles: string[] }[]): string {
...
function renderCrossAgentFindings(findings: ReporterContext['crossAgentFindings']): string {
  ...
  const rows = findings.map(f => [
    ...
    f.title,
    f.agents.join(', '),
    f.filePaths.length > 0 ? f.filePaths[0] + (f.filePaths.length > 1 ? ` (+${f.filePaths.length - 1})` : '') : 'N/A',
...
export function buildMarkdownReport(ctx: ReporterContext): string {
  ...
  lines.push(`| --- | --- | --- | --- | --- |`)
  lines.push(`| ${debt.critical} | ${debt.high} | ${debt.medium} | ${debt.low} | ${debt.total} |`)
  lines.push('')
